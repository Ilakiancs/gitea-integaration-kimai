...
class PerformanceTracker:
    ...
    def add_checkpoint(self, operation_id: str, checkpoint_name: str, data: Dict = None):
    ...
    def finish_operation(self, operation_id: str, success: bool = True, result_data: Dict = None):
        ...
        with self.lock:
            ...
            # Record overall operation performance
            self.metrics.record_performance_metric(
                f"{operation['type']}_duration",
                total_duration,
                {
            ...
            # Record checkpoint timings
            prev_time = operation['start_time']
            for checkpoint in operation['checkpoints']:
                checkpoint_duration = checkpoint['timestamp'] - prev_time
                self.metrics.record_performance_metric(
                    f"{operation['type']}_checkpoint_{checkpoint['name']}_duration",
                    checkpoint_duration,
                    {
                        'operation_id': operation_id,
                        **checkpoint['data']
        ...
        with self.lock:
            return {
                op_id: {
                    ...
                    'checkpoints': len(op_data['checkpoints']),
    ...
    def _monitor_loop(self, interval: int):
        ...
        while self.monitoring:
            try:
                ...
                self.metrics.record_performance_metric("system_cpu_percent", cpu_percent)

                # Memory usage
                memory = psutil.virtual_memory()
                self.metrics.record_performance_metric("system_memory_percent", memory.percent)
                self.metrics.record_performance_metric("system_memory_used_mb", memory.used / 1024 / 1024)

                # Disk usage
                disk = psutil.disk_usage('/')
                self.metrics.record_performance_metric("system_disk_percent", disk.percent)

                # Network I/O (if available)
                try:
                    network = psutil.net_io_counters()
                    self.metrics.record_performance_metric("system_network_bytes_sent", network.bytes_sent)
                    self.metrics.record_performance_metric("system_network_bytes_recv", network.bytes_recv)
                except:
                    pass
